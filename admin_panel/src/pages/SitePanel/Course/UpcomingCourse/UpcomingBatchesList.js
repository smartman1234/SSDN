...
export default function UpcomingBatchesList() {
  ...
  const apiCall = async (activity) => {
    try {
      let response = await batchServe.batchlist(activity);
  ...
  const name = (
    <Link to="/create-batch" className="btn btn-primary">
      <i className="fa-solid fa-plus me-2"></i>Add Batch
    </Link> 
  );
  return (
    <>
                              ...
                              {batchList?.length > 0 &&
                                batchList?.map((v, i) => (
                                  <tr key={i}>
                                    ...
                                    <td>{v.course?.name}</td>
                                    <td className="text-center">
                                      <Link to={`/enroll-list/${v.id}`}>
                                        {v.enrollment_count}
                                      </Link>
                                    </td>
                                    <td>{v.mode==="fast_track"?"Fast Track":"Normal"}</td>
                                    <td>{v.training_mode}</td>
                                    <td className="text-center">
