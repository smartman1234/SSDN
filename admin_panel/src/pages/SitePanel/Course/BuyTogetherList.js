...
export default function BuyTogetherList() {
  const comboServe = new BuyTogetherService();
  const [comboCourseList, setComboCourseList] = useState([]);
  ...
  const apiCall = async (activity) => {
    try {
      let response = await comboServe.comboCourseList(activity);
      
      if (response) {
        
        setComboCourseList(response.data);
  ...
  const comboCourseListApi = async () => {
    let activity = {
      limit: search.perPage,
      offset: search.start,
      query: search.searchTxt,
  ...
  const deleteHandle = async (dataId) => {
    try {
      if (window.confirm("Are you sure you want to delete this record?")) {
        let response = await comboServe.delete(dataId);
        if (response) {
          toast.success(response.message);
          let dataTemp = [...comboCourseList];

          const remainingData = dataTemp.filter((v) => {
            return v._id !== dataId;
          });
          comboCourseListApi();
          setComboCourseList(remainingData);
        } else {
  ...
  const toggleHandler = async (data) => {
    ...
    try {
      const res = await comboServe.changeStatus(obj);
      if (res.status === "success") {
        let items = [];
        comboCourseList.map((v) => {
          if (v.id === data.id) {
            items.push(res.data);
          } else {
            items.push(v);
  ...
  const name = (
    <Link to="/create-buy-together" className="btn btn-primary">
      <i className="fa-solid fa-plus me-2"></i>Add Buy Together
    </Link>
  );
  return (
    <>
                      ...
                      <div className="card o-hidden border-0">
                        ...
                        >
                          ...
                          <table className="table display dataTable">
                            ...
                            <tbody>
                              {comboCourseList?.length > 0 &&
                                comboCourseList?.map((v, i) => (
                                  <tr key={i}>
                          ...
                          >
                            Showing {offset + 1} to{" "}
                            {offset + comboCourseList?.length} of {totalCount}{" "}
                            entries
                          </div>
                          <div
