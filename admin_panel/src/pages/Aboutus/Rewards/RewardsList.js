...
const RewardList = () => {
  ...
  const getRewardsList = async () => {
    ...
    fetch(
      `${url}reward-recongnition/get-reward-recognition-list`,
      requestOptions
    )
  ...
  const ShowAlert = async (e, id) => {
    Swal.fire({
      ...
      if (result !== null) {
        ...
        fetch(
          `${url}reward-recongnition/delete-reward-recognition/${id}`,
          requestOptions
        )
          .then((response) => response.json())
          .then((data) => {
  ...
  const name = (
    <Link to="/reward-add" className="btn btn-primary">
      <i className="fa-solid fa-plus me-2"></i>Add Reward
    </Link>
  );
  return (
    <>
