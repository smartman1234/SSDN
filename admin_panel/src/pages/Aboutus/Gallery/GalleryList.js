...
const GalleryList = ({ gettingLength }) => {
  ...
  const getGalleryList = async () => {
    ...
    fetch(`${url}gallery/get-gallery`, requestOptions)
      .then((response) => response.json())
      .then((result) => {
        if (result.status === "success") {
          gettingLength(result?.data?.length);
  ...
  const ShowAlert = async (e, id) => {
    Swal.fire({
      ...
      if (result !== null) {
        ...
        fetch(`${url}gallery/delete-gallery/${id}`, requestOptions)
          .then((response) => response.json())
          .then((data) => {
                    ...
                    {Gallery?.length > 0 &&
                      Gallery?.map((value, key) => (
                        <tr key={key} className="align-middle">
                          ...
                          <td className="text-center">{value?.admin?.name}</td>
                          <td className="text-center">
